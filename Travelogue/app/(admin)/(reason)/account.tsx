...
const Account = () => {
  ...
  const handleAdd = async () => {
    if (inputText.trim()) {
      // Tạo một tham chiếu đến nhánh 'reasons/account' trong Realtime Database
      const wordsRef = ref(database, 'reasons/account');
  ...
  const handleRemove = (reasonId: string) => {
    const refRemove = ref(database, `/reasons/account/${reasonId}`)
    Alert.alert(
      "Remove reason",
    ...
    <View>
      <View style={styles.addBar}>
        <TextInput
          ...
          placeholder='Add reason report for account'
        />
        <TouchableOpacity

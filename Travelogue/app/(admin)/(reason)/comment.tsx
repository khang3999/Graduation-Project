...
const Comment = () => {
  ...
  const handleAdd = async () => {
    if (inputText.trim()) {
      // Tạo một tham chiếu đến nhánh 'reasons/comment' trong Realtime Database
      const wordsRef = ref(database, 'reasons/comment');
  ...
  const handleRemove = (reasonId: string) => {
    const refRemove = ref(database, `/reasons/comment/${reasonId}`)
    Alert.alert(
      "Remove reason",
    ...
    <View>
      <View style={styles.addBar}>
        <TextInput
          ...
          placeholder='Add reason report for comment'
        />
        <TouchableOpacity

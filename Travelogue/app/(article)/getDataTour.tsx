...
export const getDataTour = async (tourId: string) => {
  const postRef = ref(database, `tours/${tourId}`);
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      //Xử lý hasshTag
      //cắt chuỗi thành mảng các tag
      const hashTag = data.hashtags || "";
      console.log(hashTag);
      const hashTagArray = hashTag.split("#").filter((item: string) => item.trim() !== "");
      ...
      return {
        ...
        images,
        money : data.money || 0,
        discountTour : data.discountTour || 0,
        hashTagArray,
        hashtagNumber: data.package.hashtag,
      };
    } else {
